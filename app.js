...
// FRACTAL NOISE
// =========================
function fractalNoise(x, y, seed = 0, octaves = 5) {
  let value = 0;
  let amplitude = 1;
  let frequency = 1;
  let maxValue = 0;
...
function terrain(x, y) {
  const base = fractalNoise(x * SCALE, y * SCALE, SEED);
...
// =========================
function getColor(h) {
  if (h < 0.3) return [0, 0, 150];
  if (h < 0.4) return [0, 100, 255];
  if (h < 0.45) return [240, 230, 140];
  if (h < 0.6) return [50, 180, 50];
  if (h < 0.75) return [100, 100, 100];
  return [255, 255, 255];
}
